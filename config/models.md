...
This file is the ledger of every model that has been put through the seat's qualification, on every card, and how it
measured. It matters because the profiles in a card's registry are configuration, not a promise: a model earns a
profile here first, with numbers, and a model that is not in this table for a card has not been measured on that
card, whatever its reputation elsewhere.

The catalogue below is every card's rows, generated by `python3 harness/profiles.py render --catalogue`:
...
Every row below carries an instrument AND a card (`measured_on`, the registry's own
...
## Measured on the Intel Arc Pro B70 32 GB (llama.cpp b10920 SYCL; vLLM 0.21 XPU; 2026-09-20/24)

The registry row is `qwen38-27b-udq6k-sycl`, the only B70 checkpoint with a full ladder; the others below have a
kit run and a llama-bench speed card, but none reached the builder bar, and they ran without a task budget, so
they are not comparable with the row.

| checkpoint | engine | decode / prefill (llama-bench, depth 0 / 8k prompt) | peak VRAM | kit, counted · refs | kit wall | status |
|---|---|---|---|---|---|---|
| Qwen3.8-27B-UD-Q6_K (Unsloth) | llama.cpp SYCL | 21.4 / 1008 tok/s | 23.35 GiB | 3/3 · 3/3 (task budget 2048, full ladder) | 1.4 h | **adopted: qwen38-27b-udq6k-sycl** |
...
| Qwen3.8-27B-Q6_K-OQ8 (Frosty40) | llama.cpp SYCL | 22.0 / 1017 | 22.13 GiB | 1/3 · 1/3 | 2.4 h | measured, not adopted |
...
| Laguna-XS-2.1 Q4_K_M (Frosty40) | llama.cpp SYCL | 90.6 / 920 | 19.67 GiB | — (server did not start: architecture `laguna`) | — | not runnable on this build |
| Nex-N2-mini Q4_K_M (Frosty40) | llama.cpp SYCL | 91.2 / 955 | 20.60 GiB | 1/3 · 3/3 | 0.8 h | measured, not adopted |
| Nex-N2-mini Q5_K_M (Frosty40) | llama.cpp SYCL | 88.8 / 982 | 23.55 GiB | 2/3 · 3/3 | 0.9 h | measured, not adopted |
| Qwen-AgentWorld-35B-A3B Q5_K_M-agentaug (Frosty40) | llama.cpp SYCL | 88.5 / 1064 | 23.55 GiB | 2/3 · 3/3 | 0.9 h | measured, not adopted |
| SIQ-1-35B Q5_K_M (Frosty40) | llama.cpp SYCL | 89.2 / 1047 | 23.55 GiB | 2/3 · 1/3 | 0.9 h | measured, not adopted |
