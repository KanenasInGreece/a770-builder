...
|---|---|---|---|---|---|---|---|---|
| `Qwen3.5-9B-Q4_K_M.gguf` | `lmstudio-community/Qwen3.5-9B-GGUF` | 262,144 / q8_0 (native) | 9.49 GiB | 43.7 / 11.6 tok/s (far end 100k) | 439 / 147 tok/s | T1: 6 tests, 122 s (instruct line); SUITE-1@1: 1 of 3 counted stages passed (backend), front end + C++ failed | **qwen35-9b-q4km-vulkan** | useful to its whole native window; 100k prompt TTFT 626 s; the same file as the display registry's `qwen35-9b-q4km-vulkan`, served in full on a card with nothing else on it |
...
| `Qwen3.8-27B-GSQ-RCO-IQ3_S.gguf` | `ISTA-DASLab/Qwen3.8-27B-GSQ-RCO-GGUF` | 196,608 / q4_0, reasoning on at effort low | 14.62 GiB, 14.82 peak at a 32k prompt | 7.9 / 4.9 tok/s (8k / 64k; 100k not run) | 71 / 42 tok/s (8k / 64k) | T1: 10 tests, 546 s (card line); SUITE-1@1: 3 of 3 counted stages passed | **qwen38-27b-iq3s-vulkan** (inference) | useful to about 98k by the four-tokens-a-second rule; the quantiser's task-lossless file, 100.2% of BF16 on its own card (AIME25 100, GPQA-Diamond 89.39 against 89.90, LiveCodeBench v6 85.71 = BF16); the largest IQ3_S window under the 15.3 cap |
| `Qwen3.8-27B-GSQ-RCO-IQ3_S.gguf` | `ISTA-DASLab/Qwen3.8-27B-GSQ-RCO-GGUF` | 100,000 / q8_0, reasoning on at effort low, budget 2048 | 14.61 GiB | 9.91 / 6.29 tok/s (8k / 64k; 100k not run) | 358 / 242 tok/s (8k / 64k) | gpu-hang-check 13/13 (2026-09-13); SUITE-1@1: 3 of 3 counted stages passed, all three reference exercises passed | **qwen38-27b-iq3s-sycl** (inference, SYCL container) | measured on `Arc A770 16 GB, llama.cpp full-intel SYCL container` — a different `measured_on` from the Vulkan rows above, so it is compared only with rows sharing it; ~25% faster decode and ~5x faster prefill than the Vulkan qwen38-27b-iq3s-vulkan row at the same depths; 131,072 does not load (the SYCL server segfaults above 100,000); `useful_ctx` 100,000 is derived from THIS row's measured 8k/64k decode (the four-tokens-a-second crossing is ~155k, capped at the window), and the depth probe is not run — a first cut, not a full ladder |
...
|---|---|---|---|
| Qwen3.5-9B Q4_K_M, 262,144 | 9.49 GiB | 43.7 / 439 tok/s | **qwen35-9b-q4km-vulkan** (inference) |
...
| gemma-4-E4B Q4_K_M, 131,072, `-fa off` | 7.33 GiB | 60 / 796 tok/s | **gemma4-8b-e4b-q4km-vulkan** (display registry; the same file, read on a card with nothing else on it) |
...
| Qwen3.8-27B GSQ-RCO IQ3_S, 114,688 | 13.14 GiB | — | **qwen38-27b-iq3xxs-vulkan** (this workstation's `builder.display.env`, under its 14.1 cap; 13.78 GiB on the display card, peaking at 14.21 during a 32k prompt) |
...
| Qwen3.8-27B GSQ-RCO IQ3_S, 196,608 | 14.62 GiB, 14.82 peak at a 32k prompt | 7.9 / 71 tok/s | **qwen38-27b-iq3s-vulkan** (inference) |
...
| Qwen3.8-27B, KV q4_1 (`qwen38-27b-iq3s-vulkan`'s window) | — | 8.1 / — tok/s | measured, not adopted: compute-bound on this card whatever the KV type (q4_0 7.9, q8_0 7.8); q4_0 stays |
| Qwen3.8-27B, KV q8_0 (`qwen38-27b-iq3s-vulkan`'s window) | — | 7.8 / — tok/s | measured, not adopted: see above |
| Qwen3.8-27B, `-ub 1024` (`qwen38-27b-iq3s-vulkan`'s window) | 14.94 GiB | — | measured, not adopted: loads under the 15.3 cap; its row is owed to a later cycle |
| Qwen3.8-27B, `-ub 2048` (`qwen38-27b-iq3s-vulkan`'s window) | 15.68 GiB | — | refused: exceeds the 15.3 cap |
| gemma-4-12B-it Q6_K, 81,920, K q8_0, V f16, `-fa off` | 15.19 GiB (15.28 peak) | 14.3 / 133 tok/s (8.7 / 138 at 73,728, 503 s to first token) | measured, not adopted: useful to its whole 81,920 by the four-tokens-a-second rule and clears the builder bar (five tests, 528 s, 17.3 tok/s decode and 171 prefill at a 17k prompt), but two files measured on this card beat it on every axis it has: the 9B (a larger window, three times the decode, 5.7 GiB less VRAM, a task in a fifth of the time) and the 35B-A3B mixture-of-experts file, itself measured and not shipped (a larger window, more speed, less VRAM, a faster task) |
| Qwen3.6-35B-A3B UD-Q4_K_XL, 131,072, 16 expert layers in host RAM (24 on the card) | 15.01 GiB | 23.8 / 158 tok/s (prefill at 17k) | measured, not adopted: does not fit under 15.3 GiB with margin; the same file with 18 expert layers in host RAM, measured above, sits at 14.1 GiB and is the reading to compare against |
...
in the table above), so the cap no longer keeps it out — but it stays out of both registries: two files measured on
this card beat it on every axis it has, the 9B (a larger window, three times the decode, less VRAM and a task in a
fifth of the time) and the 35B-A3B mixture-of-experts file, itself measured and not shipped (a larger window, more
speed, less VRAM, a faster task).
...
numbers. Every inference row and the display qwen38-27b-iq3xxs-vulkan row carry a sampling line; the display qwen35-9b-q4km-vulkan and gemma4-8b-e4b-q4km-vulkan rows were
measured at the client's default and carry none until re-measured. The display qwen38-27b-iq3xxs-vulkan row's task was measured at the
