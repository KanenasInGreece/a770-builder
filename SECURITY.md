...
the model as its writable tree — the operator's home directory itself, and the data directory, are not on that list.
The run lock's descriptor is closed before the
