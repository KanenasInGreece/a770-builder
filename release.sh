...
#   bash release.sh --notes <file.md> [--dry-run] [--major]
...
# own: it takes --major, the maintainer's explicit word.
set -euo pipefail
cd "$(dirname "$0")"
SCRIPT=skills/local-build/scripts/local-build.sh
NOTES=""; DRY=0; MAJOR=0
while [ $# -gt 0 ]; do case "$1" in --notes) NOTES="${2:?file}"; shift;; --dry-run) DRY=1;; --major) MAJOR=1;; *) echo "usage: release.sh --notes <file.md> [--dry-run] [--major]" >&2; exit 2;; esac; shift; done
