...
else on it) — each with its own profile registry naming the qualified models (`config/registry/<card>.<mode>.json`). It also holds the profiling kit and its suite (`kit/`, `harness/run_suite.sh`)
...
Every card's measured rows, the whole tested family, generated by `python3 harness/profiles.py render --catalogue`:

<!-- catalogue:begin -->
| card | mode | profile | model | window (useful) | VRAM | decode / prefill at 8k | measured_on |
|---|---|---|---|---|---|---|---|
| a770 | display | gemma4-8b-e4b-q4km-vulkan | gemma-4-E4B-it-Q4_K_M.gguf | 131,072 (~100k) | 8.1 GiB | 60 / 796 tok/s | Arc A770 16 GB, llama.cpp b10805 Vulkan |
| a770 | display | qwen35-9b-q4km-vulkan | Qwen3.5-9B-Q4_K_M.gguf | 262,144 (~65k) | 10.35 GiB | 36.8 / 571 tok/s | Arc A770 16 GB, llama.cpp b10805 Vulkan |
| a770 | display | qwen38-27b-iq3xxs-vulkan | Qwen3.8-27B-GSQ-RCO-IQ3_XXS.gguf | 131,072 (~32k) | 12.25 GiB | 8.1 / 72 tok/s | Arc A770 16 GB, llama.cpp b10805 Vulkan |
| a770 | inference | qwen35-9b-q4km-vulkan | Qwen3.5-9B-Q4_K_M.gguf | 262,144 (~262k) | 9.49 GiB | 29.3 / 385.9 tok/s | Arc A770 16 GB, llama.cpp b10805 Vulkan |
| a770 | inference | qwen38-27b-iq3s-sycl | Qwen3.8-27B-GSQ-RCO-IQ3_S.gguf | 100,000 (~100k) | 14.61 GiB | 9.91 / 358.35 tok/s | Arc A770 16 GB, llama.cpp full-intel SYCL container |
| a770 | inference | qwen38-27b-iq3s-vulkan | Qwen3.8-27B-GSQ-RCO-IQ3_S.gguf | 196,608 (~98k) | 14.62 GiB | 7.9 / 71 tok/s | Arc A770 16 GB, llama.cpp b10805 Vulkan |
| b70 | inference | qwen38-27b-udq6k-sycl | Qwen3.8-27B-UD-Q6_K.gguf | 100,000 (~100k) | 23.35 GiB | 19.11 / 806.44 tok/s | Arc Pro B70 32 GB, llama.cpp full-intel SYCL container, build 10920 |
<!-- catalogue:end -->
...
| the model files of the mode's registry (`config/registry/<card>.<mode>.json`) | the profiles measured on this machine's card, in its mode | Hugging Face: [`lmstudio-community/Qwen3.5-9B-GGUF`](https://huggingface.co/lmstudio-community/Qwen3.5-9B-GGUF), [`ISTA-DASLab/Qwen3.8-27B-GSQ-RCO-GGUF`](https://huggingface.co/ISTA-DASLab/Qwen3.8-27B-GSQ-RCO-GGUF) and [`lmstudio-community/gemma-4-E4B-it-GGUF`](https://huggingface.co/lmstudio-community/gemma-4-E4B-it-GGUF); the download lines are in [`docs/OPERATING.md`](docs/OPERATING.md#getting-llamacpp-and-the-models); every model measured, on every card, is in [`config/models.md`](config/models.md), and a new one enters by the procedure in `docs/OPERATING.md` | `A770B_MODELS` (default `~/LLM/tested`), per-card `A770B_<CARD>_MODEL` overrides |
