...
# With A770B_SERVE=compose the same argv runs as a one-shot `docker compose run` of /app/llama-bench in the
# full-vulkan image: the server must be down first, and the model is read at /models/<basename>.
set -euo pipefail
...
# the flags shared by both placements; only the model path and the runner differ
COMMON=(-ngl 99 -fa "$FA" -ctk "$KV" -ctv "$KV_V" -b "$A770B_BATCH" -ub "$A770B_UBATCH" \
      --load-mode none -p "$PROMPT" -n "$GEN" -d "$DEPTHS" -r "$REPS" -o json)
[ -n "$NCMOE" ] && COMMON+=(--n-cpu-moe "$NCMOE")

if [ "$A770B_SERVE" = compose ]; then
  # the same argv, run as a one-shot in the full-vulkan image; the model is read at /models/<basename>
  ARGS=(-m "/models/$(basename "$MODEL")" "${COMMON[@]}")
  ...
  bash "$A770B_SERVE_SCRIPT" bench -- "${ARGS[@]}" > "$RAW_FILE"
  BUILD_ID="container"
else
