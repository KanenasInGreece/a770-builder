...
if [ -n "${A770B_CORPUS_FILE:-}" ] && [ -f "$A770B_CORPUS_FILE" ]; then
  cp "$A770B_CORPUS_FILE" "$CORPUS"; echo "corpus: kit generator ($A770B_CORPUS_FILE)"
else
  find "$A770B_SEAT" -name '*.py' -size +2k | head -400 | xargs cat 2>/dev/null | tr -cd '\11\12\15\40-\176' > "$CORPUS"; echo "corpus: seat glob ($A770B_SEAT/**/*.py, today's fallback — kit/corpus.py has not generated $A770B_CORPUS_FILE)"
fi
...
for want in "${SIZES[@]}"; do
  chars=$((want*4))
  if [ "$corpus_bytes" -lt "$chars" ]; then echo "⛔ corpus has $corpus_bytes bytes, need $chars for target $want tokens — skipping (head -c would silently short-fill)" >&2; continue; fi
