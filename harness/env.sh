...
_a770b_profile_lines=$(python3 "$A770B_PROJECT/harness/profiles.py" env --file "${A770B_PROFILES_FILE:-$A770B_PROJECT/config/profiles.json}") || { echo "⛔ config/profiles.json is invalid (python3 harness/profiles.py check says why)" >&2; return 2 2>/dev/null || exit 2; }
