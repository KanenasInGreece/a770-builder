...
A row in the registry (`config/registry/<card>.<mode>.json`) is not a claim; it is a
...
`kv`/`flash_attention`/`reasoning` what was actually served · `vram_gib_after_load` the after-load VRAM reading ·
`ram_gb_extra` the host RAM llama.cpp holds by design (the server log's `*_Host … buffer size` lines, at `-lv 4`) ·
`depth_probe_100k`/`task_t1` pass/fail of two rungs · `instrument` what the row was measured on ·
...
backend per card, and per engine, weight quant and activation quant. Optional beside those: `engine`,
`checkpoint` and `kernel` (omitted = untested). A GGUF `quant` label is the weight encoding; it is not a claim
that activations or the GPU kernel are the same width. `kernel.path` and `kernel.xmx` stay `untested` until
`kernel.evidence` quotes a log line from this card. A faster prefill does not, by itself, fill `kernel`.
...
|---|---|---|---|
| `ctx` | **human** — Rung 1, the load (`harness/serve_compose.sh start <gguf> <ctx>`), only confirms it | the largest window that actually loads under the mode's cap (13.0 GiB display, 15.3 GiB inference), found by hand, counting a MoE row's `--n-cpu-moe` down until it fits; `ladder.sh` echoes back whatever `--ctx` (or the registry row) gave it | none — a property of this card and cap |
...
| `ram_gb_extra` | **human** — `ladder_row.py` never computes it; transcribed once from the server's own log, read at `-lv 4` | the sum of the log's `*_Host … buffer size = N MiB` lines (MiB / 1024) — the host RAM llama.cpp holds by design for every load, dense or MoE; not the host `MemAvailable` drop, which page cache moves run to run and the ladder now records separately as `memavailable_drop_gb` in its results JSON, not in the row | none |
...
| `instrument` | **ladder** — computed per run (`<suite id>@<kit_version>`, e.g. `SUITE-1@1`) or hand-recorded for the earlier rows (`seat: Shared_Memory@3c8e2bb`) | names what the row was actually measured on; `kit_version` lives in `kit/suite.json` and moves when the tests or graders change, not when the product version does | none — it is the comparability key `comparable_with` reads, not itself a comparison |
| `placement` | **human** — optional | `host`, `container` or `remote`: where the server ran when the row was measured | none |
...
| `tool_parser` | **human** — optional | the vLLM tool-call parser the model's chat template needs; omit on llama.cpp rows | none |
| `engine` | **human** — optional; omitted means untested | `llama.cpp` or `vllm`. The process that loads the checkpoint and schedules work. Not inferred from `backend` or the filename | none |
| `checkpoint` | **human** / **ladder** — optional; omitted means untested; if present, the three schema keys plus optional `evidence` | `format` (`gguf`, `gptq`, `awq`, `safetensors`), `weight_quant` (the file's weight encoding; on a GGUF row it must equal `quant`), `activation_quant` (`none`, `fp16`, `bf16`, `int8`, `int4`), and optional `evidence` (the path to the serve evidence file, e.g. from `ladder.sh`). `none` means the checkpoint does not quantise activations — they stay in the engine's working precision. A Q4/Q6/IQ3 GGUF is weights-only: it is not 4-bit local math, and it is not W4A16/W4A4. A different encoding of the same model (GPTQ/AWQ/AutoRound) is a different checkpoint and may need a different kernel | the maker's own statement of the file encoding, never a nickname |
| `kernel` | **human** — optional; omit until measured. If present, all three keys | `path` (`gpu`, `cpu-like`, `untested`), `xmx` (`used`, `unavailable`, `untested`), `evidence` (non-empty, transcribed from a log or a bench). Grep: llama-bench `ggml_vulkan: … matrix cores:` (capability; A770 and B70 Mesa: `none`, `int dot: 1`); `load_backend: loaded Vulkan` / `loaded SYCL`; llama-bench JSON `backends`, `gpu_info`, `n_gpu_layers`, `model_type`; llama-server `unused tensor` (extra heads in **this** file); vLLM `XPUwNa16LinearKernel` / `gptq_gemm` / `switch to gptq_marlin` (the last is NVIDIA-only). Do not fill it from the quant label and do not invent FlashInfer/CUTLASS/Marlin lines this card does not print. Speed falling toward CPU while VRAM is occupied is a kernel miss, not a `useful_ctx` miss — that floor is a window rule. Recipe: `docs/OPERATING.md` *Engine, checkpoint, kernel* | none — this card's own observation |
| `mode` | **human** — stored, required | display or inference; not parsed from `measured_on` | none |
| `task` | **human** — stored, required on a kit-instrument row, optional on a legacy `seat:` row; `ladder.sh` prints a `__TODO__` placeholder | the task class the row competes in: `code-edit`, `code-read`, `debug-test`, `prose-docs`, `os-ops`, `data-structured`, `instruction-agentic`. It is the organizing axis of a registry: uniqueness is one best row per task per class per backend per card, and per engine, weight quant and activation quant | none — the class the row is filed under |
| `evidence` | **human** — stored, required on a kit-instrument row; `ladder.sh` fills it from the suite's pass result when the task rung ran | the class test that qualified the row and its pass result, e.g. `SUITE-1@1: 3/3 stages passed (this ladder run)` | none |
| `builder_class` | computed by `profiles.py card`, never stored (neither ladder nor human — a read-time derivation) | a green task (a `suite` object at 80 percent or better, else `task_t1` pass); the useful window and speed are recorded facts, not gates. A `suite` object, when present, is authoritative over `task_t1` and is never overridden by it: with `suite.stages`, a stage counts toward the pass ratio when its own `counts_toward_pass` is not `false` (`axes` only describes what a stage was scored on and never drives the tally — no stage in `kit/suite.json` puts `"working"` in `axes`, so counting by `axes` would undercount every row), the numerator is how many counted stages have `working: true`, and the row clears the bar at 80% or better of that ratio; without `suite.stages`, `suite.passed`/`suite.runs` is used the same way. `task_t1` is consulted only when there is no `suite` object at all. | none |
