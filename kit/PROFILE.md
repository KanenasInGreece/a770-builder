...
backend per card. Optional beside those: `engine`, `checkpoint` and `kernel` (omitted = untested). A GGUF
`quant` label is the weight encoding; it is not a claim that activations or the GPU kernel are the same width.
...
| `task` | **human** — stored, required on a kit-instrument row, optional on a legacy `seat:` row; `ladder.sh` prints a `__TODO__` placeholder | the task class the row competes in: `code-edit`, `code-read`, `debug-test`, `prose-docs`, `os-ops`, `data-structured`, `instruction-agentic`. It is the organizing axis of a registry: uniqueness is one best row per task per class per backend per card | none — the class the row is filed under |
